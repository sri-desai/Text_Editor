...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		for(int i = 0; i < word.length(); ++i)
		{
			if(isLowerVowel(word.charAt(i)))
			{
				...
				isPrevTextVowel = true;
			}
			else				
			{
				isPrevTextVowel = false;
			}
		}
		
		/* if the word has more than one syllable then 'e' at the end of the word is not considered as a syllable */
		if(  word.charAt(word.length() - 1) == 'e' && !isLowerVowel(word.charAt(word.length() - 2)) && syllableCount > 1)
		{
			--syllableCount;
		}
	...
	private boolean isLowerVowel (char charToCheck)
	{
		if(charToCheck == 'a' || charToCheck == 'e' || charToCheck == 'i' ||
		   charToCheck == 'o' || charToCheck == 'u' || charToCheck == 'y' )
		{
			return true;
		}
		else
	...
	{	   
		...
		double fleschScore = 206.835 - (1.015 * (wordCount / sentenceCount)) - 
