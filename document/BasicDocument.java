...
public class BasicDocument extends Document 
{
	...
	public int getNumWords()
	{
		...
		ArrayList<String> alist = (ArrayList<String>) getTokens("[a-zA-Z]+");
...
/*		
		...
		for(int charCount = text.length() - 1; charCount >= 0; --charCount)
		{
			if(text.charAt(charCount) == '.' || 
			...
			if( (text.charAt(charCount) >= 'a' && text.charAt(charCount) <= 'z') ||
				(text.charAt(charCount) >= 'A' && text.charAt(charCount) <= 'Z')
			  )
			{
				charFound = true;	
...
*/
	...
	public int getNumSyllables()
	{
		...
		{
			syllableCount += countSyllables(alist.get(i));
		}
