...
public class NearbyWords implements SpellingSuggest {
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly )
	{
		for(int index = 0; index <= s.length(); ++index)
		{
			for(int charCode = (int)'a'; charCode <= (int)'z'; charCode++)
			{
				...
				if(index != 0)
				{
					firstPhase = s.substring(0, index);
				}
				
				if(index <= s.length() - 1)
				{
					secondPhase = s.substring(index);
				}
				
				StringBuffer sb = new StringBuffer(firstPhase + Character.toString((char)charCode) + secondPhase);
	...
	public void deletions(String s, List<String> currentList, boolean wordsOnly ) 
	{
		for(int index = 0; index < s.length(); ++index)
		{
			...
			if(index < s.length() - 1)
			{
				secondPhase = s.substring(index + 1);
			}
			
			StringBuffer sb = new StringBuffer(firstPhase + secondPhase);
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while(numSuggestions > 0 && !queue.isEmpty())
		{
			...
			List<String> list = distanceOne(wordToCheck, true);
			
			while(!list.isEmpty() && numSuggestions > 0)
			{
				String listStr = list.remove(0);
				
				if(!visited.contains(listStr))
				{
					visited.add(listStr);
